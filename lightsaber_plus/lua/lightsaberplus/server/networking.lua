...
function meta:customizeHilt(hash, left)
	...
	net.Start("saberplus-start-crystal-cuztom")
		...
		net.WriteTable(quillons)
		
		local blades = {}
		for i=1,10 do
			blades[i] = itemGetData(hash, "blade"..i, "")
		end
		net.WriteTable(blades)
		
		local bladeInners = {}
		for i=1,10 do
			bladeInners[i] = itemGetData(hash, "bladeInner"..i, "")
		end
		net.WriteTable(bladeInners)
		
		local quillonInners = {}
		for i=1,10 do
			quillonInners[i] = itemGetData(hash, "quillonInner"..i, "")
		end
		net.WriteTable(quillonInners)
		net.WriteBool(left)
	net.Send(self)
	self:anim("walk_magic",1,99999)
