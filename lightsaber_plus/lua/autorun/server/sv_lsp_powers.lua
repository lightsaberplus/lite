...
local lastReset = 0
hook.Add("Think", "2j0834iopmklf", function()
	if lastReset <= CurTime() then
		forcePowerFunction = {}
		hook.Run("lightsaberPlusAddPower")
		lastReset = CurTime() + 300 -- reset powers every 5 mins
	end
end)

concommand.Add("test_add_max_force", function(ply, cmd, args)
	ply:addMaxForce("test", 999)
end)
...
hook.Add("lightsaberPlusAddPower", "freePowerPack", function()
		...
		function(ply)
			...
			if wep:GetClass() == "lightsaber_plus" then
				...
				if not wep.isOn then
					wep:toggleOn(true)
					wep:SetWeaponHoldType(LIGHTSABER_PLUS_FORMS[ply:getsyncLightsaberPlusData("saberForm", LIGHTSABER_PLUS_DEFAULT_FORM)].hold)
					wep:SetHoldType(LIGHTSABER_PLUS_FORMS[ply:getsyncLightsaberPlusData("saberForm", LIGHTSABER_PLUS_DEFAULT_FORM)].hold)
