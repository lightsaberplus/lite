...
function meta:reduceSpeed(amt, len)
	...
	local maxSlow = LIGHTSABER_PLUS_RUNSPEED
	...
	if maxSlow < LIGHTSABER_PLUS_RUNSPEED then
	...
	if lastRunThink <= CurTime() then
		for _,ply in pairs(player.GetAll()) do
			...
			if maxSlow < LIGHTSABER_PLUS_RUNSPEED then
				...
				ply:runSpeed(LIGHTSABER_PLUS_RUNSPEED)
				ply:walkSpeed(LIGHTSABER_PLUS_WALKSPEED)
			end
		end
...
hook.Add( "PlayerFootstep", "CustomFootstep", function( ply, pos, foot, sound, volume, rf )
	if LIGHTSABER_PLUS_OPTIMIZE_FEET then
