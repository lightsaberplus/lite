...
net.Receive("saberplus-saber-sound", function(len, ply)
	...
	local maxDist = LIGHTSABER_PLUS_MAX_REACH or 150
	...
	tar:syncLightsaberPlusData("staminaPower", math.Clamp(stm - (dmg*LIGHTSABER_PLUS_BLOCK_PERC),0, 100))
	...
	if tar:IsPlayer() then
		...
		if LIGHTSABER_PLUS_COMBAT_BLOCK_ON_ATTACK then
	...
	local stm = tar:getsyncLightsaberPlusData("staminaPower", 0)
	if stm <= 0 or !tar:GetActiveWeapon():getsyncLightsaberPlusData("saberOn")  or !LIGHTSABER_PLUS_COMBAT_BLOCK_ON_ATTACK then
		if !(tar.shouldRiposte) then
			local d = DamageInfo()
			d:SetDamage(dmg)
			d:SetAttacker(ply)
	...
	tar.lastDamaged = CurTime() + LIGHTSABER_PLUS_REGEN_DELAY
	ply.lastStaminaDrain = CurTime() + LIGHTSABER_PLUS_REGEN_DELAY
...
hook.Add("EntityTakeDamage", "dgkomihdslfgh", function(ply, dmg)
	...
	if LIGHTSABER_PLUS_KILL_DAMAGE_MOD then return end
	
	ply.lastDamaged = CurTime() + LIGHTSABER_PLUS_REGEN_DELAY
	...
	if stm > 0 then
		if (y < 90) and (y > -90) or LIGHTSABER_PLUS_BLOCK_360 then
			...
			if ply.lastReaction <= CurTime() then
				...
				if LIGHTSABER_PLUS_BLOCK_360 and (y > 90 or y < -90) then
			...
			if LIGHTSABER_PLUS_FACE_ON_BLOCK then
	...
	if ply.blockTimer >= CurTime() and stm > 0 then
		...
		net.Start("saberplus-block")
			net.WriteVector(dmg:GetDamagePosition())
		net.SendPVS(dmg:GetDamagePosition())
		ply:syncLightsaberPlusData("staminaPower", math.Clamp(stm - (dmg:GetDamage()*LIGHTSABER_PLUS_BLOCK_PERC), 0, 100))
		return true
	end
