...
hook.Add("HUDPaint", "sojdimfgmosidfg", function()	
	...
	local plateW = (iconS * LIGHTSABER_PLUS_FORCE_MAX_POWERS) + ((LIGHTSABER_PLUS_FORCE_MAX_POWERS+1) * padding)
	local plateH = iconS + (padding*2)
	
	
	if #currentPowers < LIGHTSABER_PLUS_FORCE_MAX_POWERS then
	...
	if LIGHTSABER_PLUS_FORCE_HUD_TEAM then
	...
	for slot,id in pairs(currentPowers) do
		...
		if LIGHTSABER_PLUS_FORCE_POINTER then
			if slot == forcePointer then
				surface.SetDrawColor(255,255,255, 255)
...
hook.Add("Think", "omlisdfgsdfgs", function()
	if input.IsKeyDown(LIGHTSABER_PLUS_FORCE_NEXT) then
	...
	if input.IsKeyDown(LIGHTSABER_PLUS_FORCE_PREV) then
	...
	if input.IsKeyDown(LIGHTSABER_PLUS_FORCE_CAST) then
...
function powerMenu()
	...
	local mx = LIGHTSABER_PLUS_FORCE_MAX_POWERS
	...
	for id,pwr in pairs(powers) do
		...
		curCount = curCount + 1
		if curCount >= LIGHTSABER_PLUS_FORCE_MAX_POWERS then
			curCount = 0
			curRow = curRow + 1
		end
	end
