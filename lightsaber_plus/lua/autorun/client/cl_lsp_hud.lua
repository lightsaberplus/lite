...
hook.Add("HUDPaint", "soimjdfghdfgh", function()
	if (LIGHTSABER_PLUS_KILL_HUD) then return end
	...
	--LIGHTSABER_PLUS_MAX_FORCE[ply:Team()] = LIGHTSABER_PLUS_MAX_FORCE[ply:Team()] or 0
	
	--local stmPerc = staminaLerp / 100
	--local frcPerc = forceLerp / LIGHTSABER_PLUS_MAX_FORCE[ply:Team()]
	
	--draw.RoundedBox( barHeight/2, 10, ScrH() * LIGHTSABER_PLUS_HUD_PERC, barLength, barHeight, Color(17,17,17,200))
	--draw.RoundedBox( (barHeight-(subBarSize))/2, 10 + (subBarSize/2), ScrH() * LIGHTSABER_PLUS_HUD_PERC + (subBarSize/2), (barLength - subBarSize)*stmPerc, barHeight-subBarSize, Color(177,177,0))	
	
	--draw.RoundedBox( barHeight/2, 10, ScrH() * LIGHTSABER_PLUS_HUD_PERC - barHeight, barLength, barHeight, Color(17,17,17,200))
	--draw.RoundedBox( (barHeight-(subBarSize))/2, 10 + (subBarSize/2), ScrH() * LIGHTSABER_PLUS_HUD_PERC + (subBarSize/2) - barHeight, (barLength - subBarSize)*frcPerc, barHeight-subBarSize, Color(200,50,255))	
	
	
	if LIGHTSABER_PLUS_ANIME_NUMBERS then
		animeNumbers()
	end
end)
...
hook.Add("HUDPaint", "LS+ XP Bar", function()
	...
	local stm = ply:getsyncLightsaberPlusData("staminaPower", 0)
	drawOutlineBar("Stamina: " .. stm .. " / " .. 100, 25, ScrH() * LIGHTSABER_PLUS_HUD_PERC - 50, 200, 15, stm / 100)
	drawOutlineBar("Force: " .. ply:getForce() .. " / " .. ply:getMaxForce(), 25, ScrH() * LIGHTSABER_PLUS_HUD_PERC, 200, 15, ply:getForce() / ply:getMaxForce())
	...
	for i=1,5 do
		...
		local form = ply:getsyncLightsaberPlusData("currentForm", LIGHTSABER_PLUS_DEFAULT_FORM)
