...
function drawBlade(item, wep, ply, name, pos, ang, tarLen, color, innerColor)
	...
	local fadeLength = LIGHTSABER_PLUS_SABER_TRAIL_SPEED
	local fadeSep = 5
	...
	-- Trail Sanity
	if LIGHTSABER_PLUS_SABER_TRAIL then
...
local savedPrint = nil

function debugPrint(m)
	--if savedPrint != m then
		print(m)
	--end
	savedPrint = m
end
...
hook.Add( "PostDrawTranslucentRenderables", "4222222222222222222222222222g", function(ply)
	if LIGHTSABER_PLUS_DRAW_SABERS then
