function runSaberTrace(ply, bladeID, pos, ang, id)
	if (LIGHTSABER_PLUS_KILL_SABER_TRACING) then return end
	if !(ply == LocalPlayer()) then return end
	local bladeLength = 35
	local segments = LIGHTSABER_PLUS_HIT_SCAN_SEGMENTS
	...
	for i=0,segments do
		...
		local segmentLength = (bladeLength/i)
