...
hook.Add("CalcView", "CL_Secondpersonz44", function(ply, pos, ang, fov)
	if LIGHTSABER_PLUS_KILL_VIEW_MOD then return end
	...
	if ply:getsyncLightsaberPlusData("crafting", false) then
		...
		if LIGHTSABER_PLUS_THIRDPERSON then
			...
			if LIGHTSABER_PLUS_CINEMATIC_CAMERA then
			...
			if LIGHTSABER_PLUS_FIRSTPERSON_REALISM then
				if LIGHTSABER_PLUS_FIRSTPERSON_REALISM_SMOOTHED then
			...
			if LIGHTSABER_PLUS_FIRSTPERSON_REALISM_LITE then
				if LIGHTSABER_PLUS_FIRSTPERSON_REALISM_SMOOTHED then
...
hook.Add("HUDPaint", "sdomirfgh", function()
	if thirdpersonConflicted and not LIGHTSABER_PLUS_KILL_VIEW_MOD then
