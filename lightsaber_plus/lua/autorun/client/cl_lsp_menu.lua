		...
		if item then
			if item.isCrystal then
				...
				bar.Paint = function(s,w,h)
					...
					if item.rarity then
						local rgb = LIGHTSABER_PLUS_RARITIES[item.rarity]
						local r = (rgb.r + 1) / 10
						local g = (rgb.g + 1) / 10
						local b = (rgb.b + 1) / 10
					...
					if item.rarity then
						draw.DrawText(item.name, "xozziesNoodle2", w/2, ScreenScale(7), LIGHTSABER_PLUS_RARITIES[item.rarity], TEXT_ALIGN_CENTER)
						draw.DrawText(item.rarity, "cooldude", w/2, 5, LIGHTSABER_PLUS_RARITIES[item.rarity], TEXT_ALIGN_CENTER)
					else
	...
	f.Paint = function(s,w,h)
		...
		for id,att in pairs(saber:GetAttachments() or {}) do
			if  string.match( att.name, "blade(%d+)" ) then
				...
				positionsInner[id] = {x = pos.x + lineLength*2, y = pos.y}
		...
		for id,att in pairs(saber:GetAttachments() or {}) do
			if  string.match( att.name, "quillon(%d+)" ) then
				...
				positions2[id] = {x = pos.x - lineLength - lineLength, y = pos.y + lineLength}
				
				surface.SetDrawColor(uiColor4.r,uiColor4.g,uiColor4.b,255)
				surface.DrawLine(pos.x, pos.y, pos.x - lineLength, pos.y - lineLength)
				surface.DrawLine(pos.x - lineLength, pos.y - lineLength, pos.x - lineLength - lineLength, pos.y - lineLength)
				
				positionsInner2[id] = {x = pos.x - lineLength - lineLength, y = pos.y - lineLength}
...
function edit3p()
	...
	cb:Dock(TOP)
	cb:SetValue(LIGHTSABER_PLUS_CINEMATIC_CAMERA)
	cb:SizeToContents()
	cb.OnChange = function(self, value)
		LIGHTSABER_PLUS_CINEMATIC_CAMERA = !LIGHTSABER_PLUS_CINEMATIC_CAMERA
	end
end